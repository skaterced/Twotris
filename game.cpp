...
void control1P(Piece* P){
  if ((ab.justPressed(LEFT_BUTTON))&&(ab.justPressed(RIGHT_BUTTON))){
    ab.setCursor(40,20);
    ab.print(F("* pause *"));
    ab.display();
    delay(100);
    while(!ab.pressed(DOWN_BUTTON));
  }
  if (P->p1){
    ...
    if ((ab.justReleased(LEFT_BUTTON))||(ab.justReleased(RIGHT_BUTTON))){
      buttonTimerMoveP1=0;
    }
